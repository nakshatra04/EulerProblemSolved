
import java.math.BigInteger;

/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */

/**
 *
 * @author naksh
 */
public class Problem8 {
    
    public static void main (String[] args)
    {
        int n = 1000;
        BigInteger number = new BigInteger("");
        BigInteger d = new BigInteger("10000000000000");
        //BigInteger d = new BigInteger("10000");
        BigInteger r;
        int mul = 1, largest = 1;
        //System.out.println(number.divide(d));
        for (int i = 1; i <=n-12;i++)
        {
            // saving last 13 digit in r
            mul = 1;
            r = number.remainder(d);
        
            for (int j = 1; j<=13 ;j++ )
            {            
                mul = mul * r.remainder(BigInteger.TEN).intValue();
                r = r.divide(BigInteger.TEN);            
            }
            //System.out.println("" + i+".   M = " +largest );
            if (mul > largest)
            {
                System.out.println(i+ ".    Largest is = " +largest);
                largest = mul;        
            }        
            
                // removing last digit from BigInteger to further work
            number = number.divide(BigInteger.TEN);        
        }
        
        System.out.println("Largest is = " +largest);
        
    }
    
}
